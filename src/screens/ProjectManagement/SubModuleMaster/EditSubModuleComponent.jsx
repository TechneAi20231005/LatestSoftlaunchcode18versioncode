...
export default function EditModuleComponent({ match }) {
    ...
    const loadData = async () => {
        const data = [];
        await new SubModuleService().getSubModuleById(subModuleId).then(res => {
            if (res.status === 200) {
                ...
                new ErrorLogService().sendErrorLog("SubModule", "Get_SubModule", "INSERT", res.message);
            }
        }).catch(error => {
            const { response } = error;
            const { request, ...errorObject } = response;
            new ErrorLogService().sendErrorLog("SubModule", "Get_SubModule", "INSERT", errorObject.data.message);
        });
        ...
        await new ModuleService().getModule().then(res => {
            if (res.status === 200) {
                if (res.data.status == 1) {
                    ...
                    setModulesDropdown(res.data.data && res.data.data.filter(d => d.is_active === 1).map(d => ({ value: d.id, label: d.module_name })))
                    //setProjectdropdown(res.data.data.filter(d => d.is_active === 1).map(d => ({ value: d.id, label: d.project_name })));
                }
            }
        })
                                    ...
                                    <div className="form-group row mt-2">
                                        ...
                                        <div className="col-sm-4">
                                            ...
                                            <Select
                                                ...
                                                defaultValue={data && Projectdropdown.filter(d=> d.value === data.project_id)}
                                                required
                                            />
                                            }
                                        </div>
                                        ...
                                        <div className="col-sm-4">
                                         ...
                                         {modulesDropdown &&   
                                            <Select
                                                ...
                                                name="module_id"
                                                required
                                                defaultValue={data && modulesDropdown && modulesDropdown.filter(d=> d.value === data.module_id)}
                                            />
                                         }
