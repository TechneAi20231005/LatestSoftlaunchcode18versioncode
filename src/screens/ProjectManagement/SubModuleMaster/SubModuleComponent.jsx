...
function SubModuleComponent({ location }) {
    ...
    const columns = [
        {
            ...
            cell: (row) =>
                <div className="btn-group" role="group" >
                    <Link to={`/${_base}/SubModule/Edit/` + row.id}
        ...
        { name: 'Sub Module Name', selector: row => row.sub_module_name, sortable: true, },
    ...
    return (
        <>
            {data &&
                ...
                >
                    {props.defaultValue == 0 && <option value={0} selected>Select Sub Module</option>}
                    {props.defaultValue != 0 && <option value={0}>Select Sub Module</option>}
                    {data.map(function (item, i) {
                        if (props.defaultValue && props.defaultValue == item.id) {
                            return <option key={i} value={item.id} selected>{item.sub_module_name}</option>
                        } else {
                            return <option key={i} value={item.id}>{item.sub_module_name}</option>
                        }
                    })
                    }
                </select>
