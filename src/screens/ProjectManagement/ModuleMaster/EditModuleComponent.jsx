...
export default function EditModuleComponent({ match }) {
    ...
    const loadData = async () => {
        ...
        await new ModuleService().getModuleById(moduleId).then(res => {
            if (res.status === 200) {
                ...
                new ErrorLogService().sendErrorLog("Module", "Get_Module", "INSERT", res.message);
            }
        }).catch(error => {
            const { response } = error;
            const { request, ...errorObject } = response;
            new ErrorLogService().sendErrorLog("Module", "Get_Module", "INSERT", errorObject.data.message);
        });
    }
    ...
    const handleForm = async (e) => {
        ...
        await new ModuleService().updateModule(moduleId, formData).then(res => {
            if (res.status === 200) {
                if (res.data.status === 1) {
                    history.push({
                ...
                new ErrorLogService().sendErrorLog("Module", "Edit_Module", "INSERT", res.message);
            }
        }).catch(error => {
            const { response } = error;
            const { request, ...errorObject } = response;
            setNotify({ type: 'danger', message: errorObject.data.message });
            new ErrorLogService().sendErrorLog("Module", "Edit_Module", "INSERT", errorObject.data.message);
        });
                        ...
                        <form onSubmit={handleForm} >
                            <div className='card mt-2'>
                                <div className='card-body'>
                                    ...
                                    <div className="form-group row mt-2">
                                        <label className="col-sm-2 col-form-label">
                                            <b>Module Name : <Astrick color="red" size="13px" /></b>
                                        </label>
                                        <div className="col-sm-4">
                                            <input type="text" className="form-control form-control-sm"
                                                id="module_name" name="module_name"
                                                required={true} defaultValue={data.module_name}
                            ...
                            <div className="mt-3" style={{ 'textAlign': 'right' }}>
                                {checkRole && checkRole[20].can_update === 1 ? <button type="submit" className="btn btn-sm btn-primary">Update</button> : ""}
                                <Link to={`/${_base}/Module`} className="btn btn-sm btn-danger text-white">Cancel</Link>
                            </div>
