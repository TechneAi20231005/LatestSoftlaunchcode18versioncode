...
function ModuleComponent({ location }) {
    ...
    const columns = [
        {
            name: "Action", width:"5%", selector: (row) => { }, sortable: false,
            cell: (row) =>
                <div className="btn-group" role="group" >
                    <Link to={`/${_base}/Module/Edit/` + row.id}
        ...
        { name: 'Module Name', width:"15%", selector: row => row.module_name, sortable: true, },
        { name: 'Project Name',  width:"15%",selector: row => row.project_name, sortable: true, },
        {
            name: "Status", width:"10%", selector: (row) => row.is_active, sortable: false,
            cell: (row) => <div>
        ...
        { name: 'Updated At', width:"10%", selector: row => row.updated_at, sortable: true, },
    ];
    ...
    return (
        <div className="container-xxl">
            ...
            <PageHeader headerTitle="Module Master" renderRight={() => {
                return <div className="col-auto d-flex w-sm-100">
                    {checkRole && checkRole[20].can_create === 1 ?
                        <Link to={`/${_base}/Module/Create`} className="btn btn-dark btn-set-task w-sm-100">
    ...
    return (
        <>
            {data &&
                ...
                >
                    {props.defaultValue == 0 && <option value="" selected>Select Module</option>}
                    {props.defaultValue != 0 && <option value="">Select Module</option>}
                    {data.map(function (item, i) {
                        if (props.defaultValue && props.defaultValue == item.id) {
                            return <option key={i} value={item.id} selected>{item.module_name}</option>
                        } else {
                            return <option key={i} value={item.id}>{item.module_name}</option>
                        }
                    })
                    }
                </select>
