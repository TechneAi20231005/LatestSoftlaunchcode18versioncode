...
export default function CreateTicketComponent() {
  ...
  const handleForm = async (e) => {
    ...
    const selectedFile = formData.get('bulk_upload_file')
  ...
  const handleGetQueryTypeForm = async (e) => {
    var data = customerMapping.filter(
      (val) => val.query_type_id == e.target.value
    );
    setRows(null);
  ...
  const loadData = async () => {
    ...
    new DepartmentMappingService()
      .getDepartmentMappingByEmployeeId(user.userId)
      .then((resp) => {
        if (resp.data.status === 1) {
          setUserDepartments(
              ...
              <div className="form-group row ">
                ...
                <div className="col-sm-3">
                  ...
                  <select
                    ...
                    required
                    onChange={(e) => {
                      handleAutoChanges(e, "Select", "query_type_id");
                  ...
                  >
                    ...
                    {queryType &&
                      queryType.map((q) => {
                        return (
                          <option value={q.id}>{q.query_type_name}</option>
                        );
                      })}
                  </select>
                </div>
              ...
              <div className="row">
                {rows.map((data, index) => {
                  return (
                    <RenderDynamicForm
                      data={data}
                      dynamicChangeHandle={dynamicChangeHandle}
                    />
                  );
                })}
              </div>
