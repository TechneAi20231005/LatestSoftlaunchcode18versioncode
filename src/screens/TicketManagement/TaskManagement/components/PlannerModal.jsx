                                            ...
                                            return <tr className="p-1">
                                                ...
                                                <td className="p-1">
                                                    ...
                                                    <select
                                                        ...
                                                        {
                                                            props.plannerData.data &&
                                                            props.plannerData.data.reduce((acc, curr) => {

                                                                const existingUser = acc.find(user => user.user_id === curr.user_id);


                                                                if (!existingUser) {
                                                                    acc.push({ user_id: curr.user_id, username: curr.username });
                                                                }

                                                                return acc;
                                                            }, []).map(user => (
                                                                <option key={user.user_id} value={user.user_id} selected={user.user_id === ele.user_id}>
                                                                    {user.username}
                                                                </option>
                                                            ))
                                                        }
...
// import { UpdatePlannerData } from '../../BasketManagement/Slices/PlannerAction';
// import PlannerSlice from '../../BasketManagement/Slices/PlannerSlice';
// function PlannerModal(props) {
...
//     const Notify = useSelector(PlannerSlice => PlannerSlice.planner.notify)
...
//         //  }
//         // setNotify(null);
//         dispatch(UpdatePlannerData({ taskId: plannerData.taskId, payload: data }))
//         // await updateTaskPlanner(plannerData.taskId, data).then(res => {
//         //     if (res.status === 200) {
//         //         if (res.data.status === 1) {
//         //             setNotify({ type: "success", message: res.data.message });
...
//                                     </thead>
//                                     <tbody>
//                                         {plannerData.data.length > 0 && plannerData.data.map((ele, index) => {
//                                             return <tr className="p-1">
//                                                 <td className="p-1 text-center">{index + 1}
//                                                     <input type="hidden" name="id[]" defaultValue={ele.id} />
//                                                 </td>
