...
const [expectedTrue,setExpectedTrue] = useState()
    const handleForm = async (e) => {
        ...
        if (data.status_id == 3 && proceed == false && (data.confirmation_required == 1 || data.confirmation_required == "1")) {
            await handleConfirmationModal(true, data);
        } else {
            setNotify(null);
            await new MyTicketService().updateTicket(ticketId, formData).then(res => {
                ...
                new ErrorLogService().sendErrorLog("Ticket", "Update_Ticket", "INSERT", errorObject.data.message);
            });
        }
    }
    ...
    const loadData = async () => {
        ...
        await new UserService().getUserForMyTickets(inputRequired).then((res) => {
                ...
                if (res.data.status == 1) {
                    ...
                    const select = res.data.data.filter(d => d.is_active == 1).map(d => ({ value: d.id, label: d.first_name + " " + d.last_name + " (" + d.id + ")" }));
                    setUser(data);
        ...
        await new MyTicketService().getTicketById(ticketId).then(res => {
            if (res.status === 200) {
                ...
                if (rows) {
                    ...
                    Promise.all(filteredArray.map(d => new DynamicFormDropdownMasterService()
                            ...
                            result.forEach((resu, i) => {
                                if (resu.status == 200) {
                                    if (resu.data.status == 1) {
                                        var temp = [];
                                        temp = resu.data.data.dropdown.map(d => ({ value: d.id, label: d.label }));
        ...
        await new DesignationService().getdesignatedDropdown().then((res) => {
            if (res.status === 200) {
                if (res.data.status == 1) {
                    ...
                    setBa(deta.BA.filter((d) => d.is_active === 1).map(d => ({ value: d.id, label: d.first_name + "-" + d.last_name + " (" + d.id + ")" })));
                    setDev(deta.DEV.filter((d) => d.is_active === 1).map(d => ({ value: d.id, label: d.first_name + "-" + d.last_name + " (" + d.id + ")" })));
                    setTester(deta.TESTER.filter((d) => d.is_active === 1).map(d => ({ value: d.id, label: d.first_name + "-" + d.last_name + " (" + d.id + ")" })));
                }
            }
        })
        ...
        await new DepartmentService().getDepartment().then(res => {
            if (res.status == 200) {
                if (res.data.status == 1) {
                    const data = res.data.data.filter(d => d.is_active == 1);
                    const select = res.data.data.filter(d => d.is_active == 1).map(d => ({ value: d.id, label: d.department }));
                    setDepartment(data);
    ...
    const handleDepartment = (e) => {
        if (e) {
            const select = user.filter(d => d.department_id == e.value)
                .map(d => ({ value: d.id, label: d.first_name + " " + d.last_name + "(" + d.id + ")" }))
            setUserDropdown(select);
            setUserName(null);
        }
    }
    ...
    const handleProjectChange = async (e) => {
        ...
        await new ProjectService().getReviewersByProject(e.value).then((res) => {
            if (res.status === 200) {
                if (res.data.status == 1) {
                    setReviewerData(res.data.data.map(d => ({ value: d.user_id, label: d.employee_name })))
                }
            }
        })
    }
    ...
    const uploadAttachmentHandler = (e, type, id = null) => {
        if (type === "UPLOAD") {
            ...
            for (var i = 0; i < e.target.files.length; i++) {
                tempSelectedFile.push({ file: e.target.files[i], show_to_customer: 0, show_to_project_owner: 0 });
            }
...
const option = [{
    ...
    useEffect(() => {
        if (user && data !== null) {
            const userData = user.map(d => ({ value: d.id, label: d.first_name + " " + d.last_name }));
        ...
        <div className="container-xxl">
            ...
            <div className="row">
                <div className="col-md-9">
                    ...
                    {data && <form onSubmit={handleForm} method="post" encType='multipart/form-data'>
                                ...
                                <div className="form-group row d-flex justify-content-between">
                                    <div className="col-sm-4">
                                        ...
                                        {queryType && <Select
                                            ...
                                            name="query_type_id"
                                            options={queryType}
                                            defaultValue={data && queryType.filter(d => d.value == data.query_type_id)}
                                            menuIsOpen={false}
                                        />
                                        }
                                    </div>
                                    ...
                                    <div className="col-sm-4">
                                        <p style={data.passed_status == "UNPASS" ? {
                                            color: "red",
                                            fontWeight: "600",
                                            fontSize: "20px"
                                        } : { color: "green", fontWeight: "600", fontSize: "20px" }}>
                                            {data.passed_status}
                                        </p>
                        ...
                        <div className='card mt-2'>
                            <div className='card-body'>
                                <div className="form-group row ">
                                    <div className="col-sm-4">
                                        <label className="col-form-label">
                                            <b>Ticket Date : <Astrick color="red" size="13px" /></b>
                                        </label>
                                        <input type="date" className="form-control form-control-sm"
                                            id="ticket_date" name="ticket_date"
                                    ...
                                    <div className="col-sm-4">
                                        <label className="col-form-label">
                                            <b>Expected Solve Date : {expectedTrue && expectedTrue == true ? <Astrick color="red" size="13px" />: ""}</b>
                                        </label>
                                        <input
                                            type="date"
                                        ...
                                        />
                                        {dateErr && dateErr == true && expectedTrue == true && (<span style={{ color: "red" }}>
                                            Please Select Date
                                        </span>)}
                                    </div>
                                    <div className="col-sm-4">
                                ...
                                <div className="form-group row ">
                                    <div className="col-sm-4">
                                        ...
                                        {departmentDropdown && <Select
                                            ...
                                            defaultValue={departmentDropdown.filter(d => d.value == data.from_department_id)}
                                            isDisabled={true}
                                        />
                                        }
                                    </div>
                            ...
                            <div className='card-body'>
                                <div className="form-group row">
                                    <div className="col-sm-3">
                                        <label className=" col-form-label">
                                            <b>Project : <Astrick color="red" size="13px" /></b>
                                        </label>
                                        {projectDropdown &&
                                            <Select
                                                id="project_id"
                                    ...
                                    <div className="col-sm-3">
                                        <label className=" col-form-label">
                                            <b>Module : <Astrick color="red" size="13px" /></b>
                                        </label>
                                        {moduleDropdown && <Select
                                            ...
                                            defaultValue={moduleDropdown.filter(d => d.value == data.module_id)}
                                        />}
                                    </div>
                                    ...
                                    <div className="col-sm-3">
                                        ...
                                        {subModuleDropdown && <Select
                                            options={subModuleDropdown}
                                            id="submodule_id"
                                            name="submodule_id"
                                            ref={subModuleIdRef}
                                            defaultValue={subModuleDropdown.filter(d => d.value == data.submodule_id)}
                                    ...
                                    <div className="col-sm-3">
                                        ...
                                        <Select
                                            ...
                                            ref={reviewerIdRef}
                                            defaultValue={data && data.ticket_users.REVIEWER.map(d => ({ value: d.id, label: d.name }))}
                                        />
                            ...
                            <div className='card mt-2'>
                                <div className='card-body'>
                                    <div className="form-group row ">
                                        <div className="col-sm-3">
                                            <label className=" col-form-label">
                                                <b>Assign to Department : <Astrick color="red" size="13px" /></b>
                                            </label>
                                            {departmentDropdown && <Select
                                                id="assign_to_department_id"
                                                name="assign_to_department_id"
                                                options={departmentDropdown}
                                                onChange={e => { handleDepartment(e) }}
                                                defaultValue={departmentDropdown.filter(d => d.value == data.assign_to_department_id)}
                                                isDisabled={isSolved}
                                            />
                                            }
                                        </div>
                                        <div className="col-sm-3">
                                            ...
                                            {userDropdown &&  <Select
                                                ...
                                                defaultValue={userDropdown && data.assign_to_user_id && userDropdown.filter(d=>d.value == data.assign_to_user_id)}
                                        ...
                                        <div className="col-sm-3">
                                            ...
                                            {statusData &&
                                                <Select
                                                    ...
                                                    defaultValue={statusData.filter(d => d.value == data.status_id)}
                                                />
                                            }
                        ...
                        <div className="row mt-3">
                            <div className="col-md-12 ml-0 pl-0">
                                <label className="form-label"><b>Attachment : </b></label>
                                        ...
                                        <tbody>
                                            {selectedFile.map((ele, i) => {
                                                return <tr className="p-1">
                                                    <td className="p-1"><b>{i + 1}.</b> {ele.file.name}</td>
                                                    <td className="p-1 text-center">
                                                        <button className='btn btn-danger text-white btn-sm p-0 px-1 mt-0' type="button"
                                                            onClick={(e) => { uploadAttachmentHandler(e, "DELETE", i) }}
                                                        >
                        ...
                        <div className="d-flex justify-content-start mt-2" style={{ overflowX: 'auto' }}>
                            {attachment && attachment.map((attach, index) => {
                                return <div className="justify-content-start"
                                    style={{ marginRight: '20px', padding: '0px', width: '200px' }}>
                                    <div className="card" style={{ backgroundColor: '#EBF5FB' }}>
                                        <div className="card-header">
                                            ...
                                            <div className="d-flex justify-content-end p-0">
                                                <a href={`${_attachmentUrl + '/' + attach.path}`}
                                                    target='_blank'
                                                    className='btn btn-warning btn-sm p-0 px-1'>
                                                    <i className="icofont-download" style={{ fontSize: '12px', height: '15px' }}></i>
                                                </a>
            ...
            {confirmationModal && <Modal centered show={confirmationModal}
                onHide={(e) => {
            ...
            >
                ...
                <form onSubmit={verifyOtp} method="POST">
                    ...
                    <Modal.Body>
                        {confirmationModalDetails && <center><h3>{confirmationModalDetails.message}</h3></center>}
                        {data &&
                            <input type="hidden" className="form-control form-control-sm"
                                name="id"
                                id="id"
                        ...
                        <div className="form-group row ">
                            <div className="col-sm-12">
                                <label className=" col-form-label">
                                    <b>Enter OTP : <Astrick color="red" size="13px" /></b>
                                </label>
                                <input type="text" className="form-control form-control-sm"
                                    name="otp"
                                    id="otp"
                        ...
                        <div className="d-flex justify-content-between">
                            ...
                            <span>
                                ...
                                <button type="button" className="btn btn-danger text-white"
                                    onClick={() => { handleConfirmationModal(false, null) }} >
                                    Close
                                </button>
                            </span>
                        </div>
