...
import { getCustomerMappingData, getQueryTypeData, getTemplateData, getcustomerTypeData } from "./CustomerMappingAction";
...
let exportTempData=[]
    ...
    builder.addCase(getcustomerTypeData.pending, (state) => {
      ...
      builder.addCase(getcustomerTypeData.fulfilled, (state, action) => {
        ...
        state.notify=null
        if (payload?.status === 200 && payload?.data?.status === 1) {
          const select = payload.data.data.filter((d) => d.is_active).map((d) => ({ value: d.id, label: d.type_name }));
      ...
      builder.addCase(getQueryTypeData.fulfilled, (state, action) => {
        ...
        if (payload?.status === 200 && payload?.data?.status === 1) {
            ...
            const queryTypeDropDownData= payload.data.data.filter((d) => d.is_active === 1).map((d) => ({ value: d.id, label: d.query_type_name }))
...
state.queryTypeDropDownData=queryTypeDropDownData
      ...
      builder.addCase(getTemplateData.fulfilled, (state, action) => {
        ...
        state.notify=null
        if (payload?.status === 200 && payload?.data?.status === 1) {
let templateDropDownData= payload.data.data.map((d) => ({value: d.id, label: d.template_name }))
