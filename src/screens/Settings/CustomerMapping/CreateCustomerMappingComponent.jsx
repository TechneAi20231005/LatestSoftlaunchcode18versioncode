...
const getDynamicForm = async () =>{
  ...
  const handleQueryType = async(e) => {
    ...
    const dynamicFormDropdownTemp = dynamicForm.filter((d) => d.id == queryTypeTemp[0].form_id).map((d) => ({ value: d.id, label: d.template_name }));
  ...
  const getUser = async () => {
    ...
    dispatch(getUserForMyTicketsData(inputRequired)).then((res) => {
      ...
      if (res.payload.data.status == 1) {
        ...
        var dropwdown = res.payload.data.data.filter((d) => d.is_active == 1).map((d) => ({ value: d.id, label: d.first_name + " " + d.last_name+" ("+ d.id +")" }));
        defaultValue = [...defaultValue, ...dropwdown];
    ...
    await new UserService().getUserWithMultipleDepartment().then((res) => {
      if (res.status == 200) {
        if (res.data.status == 1) {
          ...
          const dropdown = res.data.data.filter((d) => d.is_active == 1).filter((d) => d.multiple_department_id.includes(e.value))
                                        .map((d) => ({ value: d.id, 
                ...
                <div className="row mt-2">
                  <div className="col-sm-2">
                    <label className="col-form-label">
                      <b>Confirmation Required : <Astrick color="red" size="13px" /></b>
                    </label>
                  </div>
                    ...
                    >
                      <div className="form-check">
                        <input
                          ...
                          value="1"
                          defaultChecked={ data && (data.confirmation_required == 1 || data.confirmation_required == "1" ) }
                        />
                        <label className="form-check-label"  htmlFor="confirmation_required_yes">Yes</label>
                      </div>
                      ...
                      <div className="form-check">
                        <input
                          ...
                          value="0"
                          defaultChecked={data && (data.confirmation_required == 1 || data.confirmation_required == "0" ) }
                        />
                        <label className="form-check-label"  htmlFor="confirmation_required_no">No</label>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="form-group row mt-3">
                  ...
                  <div className="form-group row mt-3">
                    <label className="col-sm-2 col-form-label">
                      ...
                      {data.approach && data.department_id && !userDropdown && <span className="mt-2" style={{marginTop:"10%",fontSize:"16px"}}>Loading.....</span>}
