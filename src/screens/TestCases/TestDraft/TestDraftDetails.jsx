...
function TestDraftDetails(props) {
  ...
  //     name: 'Action',
  //     selector: (row) => {
  //       if (!row || row.tc_id === null || row.status_name === null) return null;
  //       return (
  //         <div className="d-flex align-items-center">
  //           <i
  //             disabled={row.status_name !== 'DRAFT'}
...
>
    ...
    },
    {
      ...
      Header: (
        <span>
          ...
          <i
              className={`icofont-filter ms-2 ${
                props.isFilterApplied['platform']
                  ? 'text-warning'
                  : 'text-dark'
              }`}
            style={{ cursor: 'pointer' }}
...

    {
      ...
      Header: (
        <span>
          ...
          <i
             className={`icofont-filter ms-2 ${
              props.isFilterApplied['tyepe_name']
                ? 'text-warning'
                : 'text-dark'
            }`}
            style={{ cursor: 'pointer' }}
...

    {
      ...
      Header: (
        <span>
          ...
          <i
              className={`icofont-filter ms-2 ${
                props.isFilterApplied['severity']
                  ? 'text-warning'
                  : 'text-dark'
              }`}
            style={{ cursor: 'pointer' }}
  ...
  useEffect(() => {
    if (filterValues && searchTerm?.length === 0) {
      ...
      if (props.isFilterApplied === false) {
        localDispatch({
          type: 'SET_SELECTED_FILTER',
          payload: filterValues.map((item) => item.name)
        });
