...
function UserTaskReportComponent() {
    ...
    const columns = [
        { name: 'Sr No', selector: row => row.sr, sortable: true, width: "100px" },
        ...
        { name: 'Total Worked', selector: row => row.total_worked, sortable: true, },
    ...
    const handleForm = async (e) => {
        ...
        if (todateformat > fromdateformat) {
            ...
            var flag = 1;
            await new ReportService().getUserTaskReport(formData).then(res => {
                if (res.status === 200) {
                    const tempData = [];
