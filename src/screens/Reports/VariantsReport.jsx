...
export default function ResourcePlanningReportComponent() {
  ...
  const [checkRole, setCheckRole] = useState(null)
    ...
    const columns = [
        ...
        { name: 'Task Owner', selector: row => row.task_owner, sortable: true, width: '175px' },
        { name: 'Task Name', selector: row => row.task_name, sortable: true, width: '175px' },
        { name: 'Start Date', selector: row => row.task_start_Date, sortable: true, },
        { name: 'Hours Scheduled', selector: row => row.task_scheduled_Hours, sortable: true, },
        { name: 'Actual Worked', selector: row => row.task_actual_worked, sortable: true, },
        { name: 'Delivery Scheduled', selector: row => row.task_delivery_scheduled, sortable: true, },
        { name: 'Status', selector: row => row.task_status, sortable: true, },
        { name: 'Actual Status', selector: row => row.task_actual_status, sortable: true, },
        { name: 'Completed At', selector: row => row.task_completed_at, sortable: true, },
    ...
    const loadData = async () => {
        ...
        const data = [];
        await new ReportService().variantsReport().then(res => {
            ...
            .catch(error => {
                ...
                new ErrorLogService().sendErrorLog("VariantsReport", "Get_VariantsReport", "INSERT", errorObject.data.message);
            })
    }
    ...
    const handleForm = async (e) => {
        ...
        if (todateformat > fromdateformat) {
            ...
            var flag = 1;
            await new ReportService().variantsReport(formData).then(res => {
                if (res.status === 200) {
                    if (res.data.status == 1) {
                        let sr = 1;
                        const data = res.data.data;
                    ...
                    new ErrorLogService().sendErrorLog("VariantsReport", "Get_VariantsReport", "INSERT", res.message);
                }
            }).catch(error => {
                const { response } = error;
                const { request, ...errorObject } = response;
                ...
                <div className='card-body'>
                    <form onSubmit={handleForm}  >
                        <div className="row">
                            ...
                            <div className="col-md-3">
                                <label htmlFor="" className="">
                                    <b>From Date : <Astrick color="red" size="13px" /></b>
                                </label>
                            ...
                            <div className="col-md-3">
                                <label htmlFor="" className="">
                                    <b>To Date : <Astrick color="red" size="13px" /></b>
                                </label>
                                <input type="date" className="form-control form-control-sm"
                        ...
                        <div className="row">
                            ...
                            <div className="col-md-10" style={{ textAlign: 'right', marginTop: '20px', fontWeight: '600' }}>
                                <ExportToExcel className="btn btn-sm btn-danger" apiData={exportData} fileName="Variance Report" />
                            </div>
                        </div>
                    </form>
                </div>
