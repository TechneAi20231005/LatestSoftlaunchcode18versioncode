...
function EditCustomer({ match }) {
    ...
    const loadData = async () => {
        const data = [];
        await new CustomerService().getCustomerById(customerId).then(res => {
            if (res.status === 200) {
            ...
            new ErrorLogService().sendErrorLog("Customer", "Get_Customer", "INSERT", errorObject.data.message);
        });
        ...
        await new CustomerType().getCustomerType().then(res => {
            if (res.status === 200) {
                ...
                setCustomerType(data.filter(d => d.is_active == 1).map(d => ({ label: d.type_name, value: d.id })))
        ...
        await new CountryService().getCountrySort().then(res => {
            if (res.status === 200) {
                if (res.data.status == 1) {
                    ...
                    setCountryDropdown(res.data.data.filter(d => d.is_active == 1).map(d => ({ value: d.id, label: d.country })));
                }
            }
        })
        //  ************************** State load data**************************************
        await new StateService().getStateSort().then(res => {
            if (res.status === 200) {
                if (res.data.status == 1) {
                    ...
                    setStateDropdown(res.data.data.filter(d => d.is_active === 1).map(d => ({ value: d.id, label: d.state, country_id: d.country_id })));
                }
            }
        })
        //  ************************** city load data**************************************
        await new CityService().getCity().then(res => {
            if (res.status === 200) {
                if (res.data.status == 1) {
                    ...
                    setCityDropdown(res.data.data.filter(d => d.is_active === 1).map(d => ({ value: d.id, label: d.city, state_id: d.state_id })));
                }
            }
        })
    ...
    const handleForm = async (e) => {
        ...
        if (customerType == "" || selectEmail == "" || selectCountry == "" || selectState == "" || selectCity == "") {
            flag = 0;
            setNotify(null)
            if (customerType == "") {
                // setNotify(null);
        ...
        if (flag == 1) {
            setNotify(null);
            await new CustomerService().updateCustomer(customerId, formData).then(res => {
                if (res.status === 200) {
                    if (res.data.status === 1) {
                        history.push({
                ...
                new ErrorLogService().sendErrorLog("Customer", "Create_Customer", "INSERT", errorObject.data.message);
            });
        }
    }
    ...
    useEffect(() => {
        if (data !== null && cityDropdown !== null && updateStatus.citydrp === undefined) {
            ...
            const newStatus = { ...updateStatus, citydrp: 1 }
            setUpdateStatus(newStatus)
            setCityName(data && cityDropdown && cityDropdown.filter(d => d.value == data.city_id) ? data && cityDropdown && cityDropdown.filter(d => d.value == data.city_id) : cityName)
        }
                    ...
                    {data && <form onSubmit={handleForm}>
                            ...
                            <div className='card-body'>
                                ...
                                <div className="form-group row">
                                    <label className="col-sm-2 col-form-label">
                                        <b>Customer Name : <Astrick color="red" size="13px" /></b>
                                    </label>
                                ...
                                <div className="form-group row mt-3">
                                    <label className="col-sm-2 col-form-label">
                                        <b>Customer Type :<Astrick color="red" size="13px" /></b>
                                    </label>
                                    <div className="col-sm-4">
                                        {customerType &&
                                            <Select
                                                ...
                                                id="customer_type_id"
                                                required
                                                defaultValue={data && customerType.filter(d => d.value == data.customer_type_id)}
                                            />
                                        }
                                    </div>
                                </div>
                                ...
                                <div className="form-group row mt-3">
                                    <label className="col-sm-2 col-form-label">
                                        <b>Email Address :<Astrick color="red" size="13px" /></b>
                                    </label>
                                    <div className="col-sm-4">
                                        <input type="text" className="form-control form-control-sm"
                                            id="email_id"
                                ...
                                <div className="form-group row mt-3">
                                    <label className="col-sm-2 col-form-label">
                                        <b>Contact Number : <Astrick color="red" size="13px" /></b>
                                    </label>
                                    <div className="col-sm-4">
                                        <input type="text" className="form-control form-control-sm"
                                            id="contact_no"
                                ...
                                {contactError && <small style={{ color: "red", position: 'relative', left: "12.375rem" }}>{contactError}</small>}
                                <div className="form-group row mt-3">
                                    <label className="col-sm-2 col-form-label">
                                        <b>Remark :</b>
                                    </label>
                                            ...
                                            <div className="col-md-2">
                                                <div className="form-check">
                                                    <input className="form-check-input" type="radio" name="is_active" id="is_active_1"
                                                        value="1"
                                                        defaultChecked={(data && data.is_active) ? true : ((!data) ? true : false)}
                                                    />
                        ...
                        <div className='card mt-2'>
                            ...
                            <div className='card-body'>
                                <div className="form-group row mt-3">
                                    <label className="col-sm-2 col-form-label">
                                        <b>Address :<Astrick color="red" size="13px" /></b>
                                    </label>
