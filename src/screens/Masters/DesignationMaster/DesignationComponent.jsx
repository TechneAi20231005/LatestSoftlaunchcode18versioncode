...
function DesignationComponent() {
    ...
    const columns = [
        ...
        { name: "Designation", selector: (row) => row.designation, sortable: true, width: "150px",},
        {
    ...
    const loadData = async () => {
        ...
        await new DesignationService().getDesignation().then(res => {
            ...
            new ErrorLogService().sendErrorLog("Designation", "Get_Designation", "INSERT", errorObject.data.message);
        })
    ...
    const handleForm = id => async (e) => {
        ...
        if (!id) {
            await new DesignationService().postDesignation(form).then(res => {
                if (res.status === 200) {
                    setShowLoaderModal(false);
                    if (res.data.status === 1) {
                    ...
                    new ErrorLogService().sendErrorLog("Designation", "Create_Designation", "INSERT", res.message);
                }
            }).catch(error => {
                const { response } = error;
                const { request, ...errorObject } = response;
                setNotify({ type: 'danger', message: "Request Error !!!" })
                new ErrorLogService().sendErrorLog("Designation", "Create_Designation", "INSERT", errorObject.data.message);
            })
        } else {
            await new DesignationService().updateDesignation(id, form).then(res => {
                if (res.status === 200) {
                    setShowLoaderModal(false);
                    if (res.data.status === 1) {
                        setNotify({ type: 'success', message: res.data.message })
                    ...
                    setNotify({ type: 'danger', message: res.data.message })
                    new ErrorLogService().sendErrorLog("Designation", "Edit_Designation", "INSERT", res.message);
                }
            }).catch(error => {
                const { response } = error;
                const { request, ...errorObject } = response;
                setNotify({ type: 'danger', message: "Request Error !!!" })
                new ErrorLogService().sendErrorLog("Designation", "Edit_Designation", "INSERT", errorObject.data.message);
            })
        }
    }
    ...
    useEffect(() => {
        if(checkRole && checkRole[7].can_read === 0){
    ...
    return (
        <div className="container-xxl">
            ...
            <PageHeader headerTitle="Designation Master" renderRight={() => {
                return <div className="col-auto d-flex w-sm-100">
                    {checkRole && checkRole[7].can_create === 1 ?
                        <button className="btn btn-dark btn-set-task w-sm-100" onClick={() => { handleModal({ showModal: true, modalData: null, modalHeader: 'Add Designation' }) }}>
            ...
            <Modal centered show={modal.showModal}
                ...
                <form method="post" onSubmit={handleForm(modal.modalData ? modal.modalData.id : '')}>
                    ...
                    <Modal.Body>
                        ...
                        <div className="deadline-form">
                            ...
                            <div className="row g-3 mb-3">
                                ...
                                <div className="col-sm-12">
                                    <label className="form-label font-weight-bold">Designation Name :<Astrick color="red" size="13px" /></label>
                                    <input type="text"
                    ...
                    <Modal.Footer>
                        ...
                        {modal.modalData && checkRole && checkRole[7].can_update === 1 ? <button type="submit" className="btn btn-primary text-white" style={{ backgroundColor: "#484C7F" }}  >
                            Update
                        </button> : ""}
    ...
    return (
        <>
            {data &&
                ...
                >
                    {props.defaultValue == 0 && <option value={0} selected>Select Designation</option>}
