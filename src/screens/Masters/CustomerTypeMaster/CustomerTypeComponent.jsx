...
function CustomerTypeComponent() {
    ...
    const columns = [
        ...
        { name: "Customer Type Name", selector: (row) => row.type_name, sortable: true, width: "200px", },
        {
    ...
    const loadData = async () => {
        ...
        await new CustomerType().getCustomerType().then(res => {
            ...
            new ErrorLogService().sendErrorLog("Customer Type", "Get_CustomerType", "INSERT", errorObject.data.message);
        })
    ...
    const handleForm = id => async (e) => {
        ...
        if (!id) {
            await new CustomerType().postCustomerType(form).then(res => {
                if (res.status === 200) {
                setShowLoaderModal(false);
                    ...
                    new ErrorLogService().sendErrorLog("CustomerType", "Create_CustomerType", "INSERT", res.message);
                }
            }).catch(error => {
                const { response } = error;
                const { request, ...errorObject } = response;
                setNotify({ type: 'danger', message: "Request Error !!!" });
                new ErrorLogService().sendErrorLog("CustomerType", "Create_CustomerType", "INSERT", errorObject.data.message);
            })
        } else {
            await new CustomerType().updateCustomerType(id, form).then(res => {
                if (res.status === 200) {
                setShowLoaderModal(false);
                    ...
                    setNotify({ type: 'danger', message: res.message });
                    new ErrorLogService().sendErrorLog("CustomerType", "Edit_CustomerType", "INSERT", res.message);
                }
            }).catch(error => {
                const { response } = error;
                const { request, ...errorObject } = response;
                setNotify({ type: 'danger', message: "Request Error !!!" });
                new ErrorLogService().sendErrorLog("CustomerType", "Edit_CustomerType", "INSERT", errorObject.data.message);
            })
        }
    }
    ...
    return (
        <div className="container-xxl">
            ...
            <PageHeader headerTitle="Customer Type Master" renderRight={() => {
                return <div className="col-auto d-flex w-sm-100">
                    {checkRole && checkRole[11].can_create === 1 ?
                        <button className="btn btn-dark btn-set-task w-sm-100" onClick={() => { handleModal({ showModal: true, modalData: null, modalHeader: 'Add Customer Type' }) }}>
            ...
            <Modal centered show={modal.showModal}
                ...
                <form method="post" onSubmit={handleForm(modal.modalData ? modal.modalData.id : '')}>
                    ...
                    <Modal.Body>
                        ...
                        <div className="deadline-form">
                            ...
                            <div className="row g-3 mb-3">
                                <div className="col-sm-12">
                                    <label className="form-label font-weight-bold">Customer Type Name :<Astrick color="red" size="13px" /></label>
                                    <input type="text"
                    ...
                    <Modal.Footer>
                        ...
                        {modal.modalData && checkRole && checkRole[11].can_update === 1 ? <button type="submit" className="btn btn-primary text-white" style={{ backgroundColor: "#484C7F" }}  >
                            Update
                        </button> : ""}
                ...
                >
                    ...
                    {data.map(function (item, i) {
                        if (props.defaultValue && props.defaultValue == item.id) {
                            return <option key={i} value={item.id} selected>{item.type_name}</option>
                        } else {
                            return <option key={i} value={item.id}>{item.type_name}</option>
                        }
                    })
                    }
                </select>
