...
function DepartmentMappingComponent() {
  ...
  const columns = [
          ...
          <button
            ...
            onClick={(e) => {
              handleModal({
                showModal: true,
                modalData: row,
                modalHeader: 'Edit Department Mapping'
              });
            }}
          >
    ...
    {
      name: 'Ticket Show Type',
      selector: (row) => row.ticket_show_type,
      sortable: true
    },
  ...
  const handleForm = (id) => async (e) => {
    ...
    new DepartmentMappingService()
      .updateDepartmentMapping(id, form)
      .then((res) => {
        if (res.status === 200) {
          if (res.data.status) {
          ...
          new ErrorLogService().sendErrorLog(
            ...
            'Edit_Employee_Department_Mapping',
            'INSERT',
            res.message
          );
        }
  ...
  return (
    <div className="container-xxl">
      ...
      <PageHeader
        headerTitle="Department Mapping Master"
        renderRight={() => {
          return (
            <div className="col-auto d-flex w-sm-100">
              <Link
                to={`/${_base + '/DepartmentMapping/Create'}`}
                ...
                {modal.modalData.ticket_show_type && (
                  <div className="col-sm-12">
                    ...
                    <Select
                      ...
                      defaultValue={ticketViewOption.filter(
                        (d) =>
                          d.value ===
                          modal.modalData.ticket_show_type.replace(' ', '_')
                      )}
                    />
                  </div>
                )}
