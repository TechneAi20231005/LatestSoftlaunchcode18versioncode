...
import WhatsappService from "../../../services/ConnectorService/WhatsappService";


const WhatsappComponent = ({ location }) => {
  ...
  const loadData = async () => {
    const tempData = [];
    await new WhatsappService().getWhatsappVendors().then((res) => {
      if (res.status === 200) {
  ...
  const handleActive = async (id, type) => {
    await new WhatsappService().activeVendor(id, "WHATSAPP", type).then((res) => {
      if (res.status === 200) {
        const data = res.data.data.WHATSAPP;
        setData(data);
      }
  ...
  return (
    ...
    <div className="card">
        ...
        border-bottom-0">
            <h2 className="mb-0 fw-bold ">WHATSAPP</h2>         
        </div>
    </div>
