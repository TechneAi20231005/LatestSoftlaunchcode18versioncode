...
import SmsService from "../../../services/ConnectorService/SmsService";

const SmsComponent = ({ location }) => {
  ...
  const loadData = async () => {
    const tempData = [];
    await new SmsService().getSmsVendors().then((res) => {
      if (res.status === 200) {
        const data = res.data.data.SMS;
        setData(data);
      }
    });
  };
  ...
  const handleActive = async (id,type) => {
    await new SmsService().activeVendor(id,'SMS',type).then((res) => {
      if (res.status === 200) {
        const data = res.data.data.SMS;
        setData(data);
      }
  ...
  return (
    ...
    <div className="card">
        ...
        border-bottom-0">
            <h2 className="mb-0 fw-bold ">SMS</h2>         
        </div>
    </div>
